/**
 * Write the Cloudinary URLs and alt text into the LIVE Impact entry — media
 * fields only, every copy field left exactly as stored.
 *
 *   node scripts/apply-impact-media-to-cms.cjs --dry-run    # report, write nothing
 *   node scripts/apply-impact-media-to-cms.cjs              # apply, then publish
 *   node scripts/apply-impact-media-to-cms.cjs --restore    # roll back from the backup
 *
 * CommonJS on purpose: @strapi/strapi's ESM build fails to resolve its own
 * `lodash/fp` directory import under Node's ESM loader.
 *
 * bootstrap() only seeds an Impact entry when there isn't one, so the seed file
...
/**
 * No markup patches on this page. The net-zero progress wheel (impact/page.tsx:
 * 774-795) is built at render time from the polar()/arc() helpers, so unlike the
 * Projects footprint map there is no static SVG string to seed.
 */
const MARKUP_PATCHES = [];

/** Media fields that intentionally have no `_alt_text` sibling. */
const NO_ALT_TEXT = new Set(['video']); // <video> has no alt attribute

/** Repeatable arrays needing rows created before patching. stories[] (6) and
 *  sdgCards[] (4) are seeded by impact-page-copy.ts, so none are needed. */
const ENSURE_ITEMS = [];
 ...
 * drop it. No impact-page leaf component contains a further component, so a
 * single-level `*` covers every section.
 */
const SECTION_POPULATE = {
  'impact-page.structured-data-section': { populate: '*' },
  'impact-page.hero-section': { populate: '*' },
  'impact-page.philosophy-section': { populate: '*' },
  'impact-page.impact-console-section': { populate: '*' },
  'impact-page.stories-tab-section': { populate: '*' },
  'impact-page.numbers-tab-section': { populate: '*' },
  'impact-page.investments-tab-section': { populate: '*' },
  'impact-page.assets-tab-section': { populate: '*' },
  'impact-page.next-steps-section': { populate: '*' },
  'impact-page.video-modal-section': { populate: '*' },
};
  ...
  try {
    ...
    if (!RESTORE) {
      for (const { component, field, rows } of ENSURE_ITEMS) {
        const section = impact.sections.find((item) => item.__component === component);
    ...
    for (const patch of patches) {
      const section = impact.sections.find((item) => item.__component === patch.component);
      if (!section) {
    ...
    await strapi.documents('api::impact.impact').update({
      documentId: impact.documentId,
      data: { sections: impact.sections },
    });
