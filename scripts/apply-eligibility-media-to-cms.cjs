/**
 * Write the Cloudinary URLs and alt text into the LIVE Eligibility entry — media
 * fields only, every copy field left exactly as stored.
 *
 *   node scripts/apply-eligibility-media-to-cms.cjs --dry-run    # report, write nothing
 *   node scripts/apply-eligibility-media-to-cms.cjs              # apply, then publish
 *   node scripts/apply-eligibility-media-to-cms.cjs --restore    # roll back from the backup
 *
 * CommonJS on purpose: @strapi/strapi's ESM build fails to resolve its own
 * `lodash/fp` directory import under Node's ESM loader.
 *
 * bootstrap() only seeds an Eligibility entry when there isn't one, so the seed
 * file and the database diverge once the entry exists. This closes that gap
 * without touching the seed-once guard.
 *
 * Mirrors scripts/apply-impact-media-to-cms.cjs.
 */
...
const PAGE_NAME = 'eligibility-page';
...
/**
 * No markup patches on this page. The assessment readiness dial
 * (eligibility/assessment/page.tsx:738-770) is built at render time from
 * radius/circumference and animated by framer-motion, so — unlike the Projects
 * footprint map — there is no static SVG string to seed.
 */
const MARKUP_PATCHES = [];

/** Media fields that intentionally have no `_alt_text` sibling. */
const NO_ALT_TEXT = new Set(); // both rows are <img>-backed and carry alt text

/** Repeatable arrays needing rows created before patching. Both media fields are
 *  top-level scalars on their section, so none are needed. */
const ENSURE_ITEMS = [];
 ...
 * drop it — including the two-level nests (cards[].listItems / cards[].stats and
 * questions[].options).
 */
const SECTION_POPULATE = {
  'eligibility-page.structured-data-section': { populate: '*' },
  'eligibility-page.hero-section': { populate: '*' },
  'eligibility-page.criteria-pillars-section': {
    populate: { cards: { populate: '*' } },
  },
  'eligibility-page.timeline-workflow-section': { populate: '*' },
  'eligibility-page.next-steps-section': { populate: '*' },
  'eligibility-page.final-cta-section': { populate: '*' },
  'eligibility-page.assessment-chrome-section': { populate: '*' },
  'eligibility-page.assessment-steps-section': {
    populate: { steps: { populate: '*' }, questions: { populate: '*' } },
  },
  'eligibility-page.assessment-result-section': {
    populate: { outcomes: { populate: '*' }, logRows: { populate: '*' } },
  },
};
...
async function main() {
  ...
  try {
    const eligibility = await strapi.documents('api::eligibility.eligibility').findFirst({
      status: 'draft',
      populate: { sections: { on: SECTION_POPULATE } },
    });
    if (!eligibility?.sections?.length) throw new Error('no Eligibility draft with sections found');
    ...
    if (!RESTORE) {
      for (const { component, field, rows } of ENSURE_ITEMS) {
        const section = eligibility.sections.find((item) => item.__component === component);
    ...
    for (const patch of patches) {
      const section = eligibility.sections.find((item) => item.__component === patch.component);
      if (!section) {
    ...
    await strapi.documents('api::eligibility.eligibility').update({
      documentId: eligibility.documentId,
      data: { sections: eligibility.sections },
    });
    console.log('draft updated.');

    await strapi.documents('api::eligibility.eligibility').publish({
      documentId: eligibility.documentId,
    });
