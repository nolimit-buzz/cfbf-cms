/**
 * Read every Impact media field back out of the database and report whether it
 * actually holds a Cloudinary URL — the acceptance test for the extract →
 * upload → apply pipeline.
 *
 *   node scripts/verify-impact-media-in-cms.cjs                 # draft (default)
 *   node scripts/verify-impact-media-in-cms.cjs --status=published
 *
...
const MANIFEST_PATH = path.join(CMS_ROOT, 'seed-manifests', 'impact-page', 'manifest.json');

const status =
  process.argv.find((arg) => arg.startsWith('--status='))?.slice('--status='.length) ?? 'draft';

const NO_ALT_TEXT = new Set(['video']);
...
async function main() {
  ...
  try {
    ...
    if (!impact?.sections?.length) throw new Error(`no Impact ${status} entry with sections found`);
    ...
    for (const entry of manifest) {
      const section = impact.sections.find((item) => item.__component === entry.component);
      ...
      rows.push({
        field: `${entry.component.replace('impact-page.', '')}.${
    ...
    // See apply-impact-media-to-cms.cjs — destroy() can time out acquiring a
