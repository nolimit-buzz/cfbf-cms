/**
 * Read every Eligibility media field back out of the database and report whether
 * it actually holds a Cloudinary URL — the acceptance test for the extract →
 * upload → apply pipeline.
 *
 *   node scripts/verify-eligibility-media-in-cms.cjs                 # draft (default)
 *   node scripts/verify-eligibility-media-in-cms.cjs --status=published
 *
 * Exits non-zero if any manifest row's field or its `_alt_text` sibling is
 * missing, empty, or not a res.cloudinary.com URL.
 *
 * Mirrors scripts/verify-impact-media-in-cms.cjs.
 */
...
const MANIFEST_PATH = path.join(CMS_ROOT, 'seed-manifests', 'eligibility-page', 'manifest.json');
...
/** Only the two sections that own media need populating for this read. */
...
async function main() {
  ...
  try {
    ...
    if (!eligibility?.sections?.length) {
      throw new Error(`no Eligibility ${status} entry with sections found`);
    }
    ...
    for (const entry of manifest) {
      const section = eligibility.sections.find((item) => item.__component === entry.component);
      ...
      rows.push({
        field: `${entry.component.replace('eligibility-page.', '')}.${
    ...
    // See apply-eligibility-media-to-cms.cjs — destroy() can time out acquiring a
